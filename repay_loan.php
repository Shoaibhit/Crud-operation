...
if (isset($_POST['save_loan'])) {
    ...
    if ($old['loan_amount'] === '' || !is_numeric($old['loan_amount'])) {
        $errors['loan_amount'] = 'Valid repayment amount is required.';
    }
    if ($old['loan_date'] === '') {
        $errors['loan_date'] = 'Repayment date is required.';
    }
    if ($old['due_date'] === '') {
        $errors['due_date'] = 'Due date is required.';
    }
    if ($old['payment_method'] === '') {
        $errors['payment_method'] = 'Payment type is required.';
    }
    ...
    if (!array_filter($errors)) {
        ...
        if ($stmt) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                $_SESSION['successMessage'] = 'Repayment record saved successfully.';
        ...
        <div class="row gx-0">
            ...
            <main class="col-12 col-md-9 col-xl-10 p-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h1 class="h3 mb-1">Repay Loan</h1>
                        <p class="text-muted mb-0">Enter the repayment details for the borrower.</p>
                    </div>
                    <a href="loans.php" class="btn btn-primary btn-sm rounded-pill px-4">
                ...
                <div class="card form-card bg-white border-0 p-4">
                    <div class="mb-4 d-flex justify-content-between align-items-center">
                        <span class="badge bg-primary rounded-pill px-3 py-2 fs-6">
                            <i class="bi bi-cash-stack me-1"></i> Repay Loan
                        </span>
                    </div>
                    ...
                    <form action="" method="POST" enctype="multipart/form-data">
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                <label for="loanAmount" class="form-label">Repayment Amount</label>
                            ...
                            <div class="col-md-6">
                                <label for="loanDate" class="form-label">Repayment Date</label>
                            ...
                            <div class="col-12">
                                ...
                                <input type="file" id="screenshot" name="screenshot" class="form-control form-control-lg rounded-4" accept="image/*" />
                                <?php if ($errors['screenshot']): ?><div class="text-danger small mt-1"><?php echo htmlspecialchars($errors['screenshot']); ?></div><?php endif; ?>
                            </div>
                            <div class="col-12">
                                <label for="loanNotes" class="form-label">Notes</label>
                                <textarea id="loanNotes" name="loan_notes" class="form-control rounded-4" rows="4" placeholder="Add any extra details..."><?php echo htmlspecialchars($old['loan_notes']); ?></textarea>
                                <?php if ($errors['loan_notes']): ?><div class="text-danger small mt-1"><?php echo htmlspecialchars($errors['loan_notes']); ?></div><?php endif; ?>
                            </div>
                            <div class="col-12">
                                <button type="submit" class="btn btn-primary btn-lg rounded-pill px-5" name="save_loan">Save Repayment</button>
                            </div>
                        </div>
                    </form>
                </div>
